...
function resetBoard(order) {
  ...
  for (var i = 0; i < cryptocurrencies.length; i++) {//for loop to display cryptocurrencies[] on the table
    let percent = cryptocurrencies[i].percentage_change_24h.toFixed(2)
    let percent1 = " " + percent
    let symbol = cryptocurrencies[i].symbol.toUpperCase();
    var num1 = cryptocurrencies[i].market_cap;
    var commas1 = num1.toLocaleString("en-US");
    var num2 = cryptocurrencies[i].market_cap;
    var commas2 = num2.toLocaleString("en-US");
    if (cryptocurrencies[i].percentage_change_24h > 0) {
      var $item = $(
        ...
        "<td class='market_cap'>" + commas1 + "</td>" +
        "<td class='volume_24h'>" + commas2 + "</td>" +
        "<td class='percentage_change_24h' id = 'chart_percentage' style = 'color: #32CD32'>&nbsp" + percent1 + "%</td>" +
        "</tr>"
      );
    }
    else {
      var $item = $(
        ...
        "<td class='market_cap'>" + commas1 + "</td>" +
        "<td class='volume_24h'>" + commas2 + "</td>" +
        "<td class='percentage_change_24h' id = 'chart_percentage' style = 'color: red'>" + percent + "%</td>" +
        "</tr>"
      );
    }
  ...
  document.querySelector('#cotd_price_value').style.color = '#F7CA14'
  ...
  if (cotd_data[0].percentage_change_24h > 0) {
    document.querySelector('#cotd_percentage_value').style.color = '#32CD32'
    document.querySelector('#cotd_percentage_arrow').style.color = '#32CD32'
    document.querySelector('#cotd_percentage_sign').style.color = '#32CD32'
  }
  ...
  document.querySelectorAll('#cryptocurrencies tr').forEach(selected => {
    ...
    selected.addEventListener("click", () => {
      ...
      if (status1 == 'market_cap_inc') {
        selected_rank = 26 - selected.rowIndex;
      }
      else {
      ...
      var num3 = cryptocurrencies[26 - selected_rank - 1].market_cap
      var commas3 = num3.toLocaleString("en-US");
      var num4 =cryptocurrencies[26 - selected_rank - 1].volume_24h;
      var commas4 = num4.toLocaleString("en-US");
      if (status1 == 'market_cap_inc') {
        console.log(cryptocurrencies[26 - selected_rank - 1].name)
        document.querySelector('.modal-name').innerText = cryptocurrencies[26 - selected_rank - 1].name;
        document.querySelector('#modal_percentage_value').innerText = cryptocurrencies[26 - selected_rank - 1].percentage_change_24h.toFixed(3);
        document.querySelector('.modal-marketcap-value').innerText = commas3
        document.querySelector('.modal-price-value').innerText = "$" + cryptocurrencies[26 - selected_rank - 1].price
        document.querySelector('.modal-volume-value').innerText = commas4
      }
      else {
        console.log(cryptocurrencies[selected_rank - 1].name)
      ...
      document.querySelector('#modal-close-container').addEventListener("click", () => {
  ...
  document.querySelectorAll('#cryptocurrencies tr').forEach(selected => {
    selected.addEventListener("click", () => {
      ...
      document.getElementById('modal-close-container').innerHTML = '<i style = "position: flex; font-size: 35px;right: 13px;padding-top: 2px;padding-left: 5px;cursor: pointer;color: #fdfdfd;" class="fa-solid fa-xmark" class="modal-close" id="modal-close-' + selected_coin + '"></i>'
      document.querySelector('#modal-close-' + selected_coin).addEventListener("click", () => {

        document.getElementById('modal-close-' + selected_coin).remove()
...
resetBoard(status1);//default 1 is market_cap_dec
